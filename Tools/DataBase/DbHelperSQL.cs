...
namespace Tools
{
    ...
    public class DbHelperSQL
    {
        ...
        public DbHelperSQL(string DBContext)
        {
        ...
        /// </summary>
        /// <param name="SQLStringList">SQL语句的哈希表（key为sql语句，value是该语句的SqlParameter[]）</param>
        public void ExecuteProcedureTranWithIndentity(string strCmdTxt, List<Hashtable> sqlParList)
        {
            ...
            {
                //循环
                foreach (Hashtable table in sqlParList)
                {
                    ...
                    foreach (DictionaryEntry myDE in table)
                    {
                        ...
                        {
                            ParameterName = myDE.Key.ToString(),
                            Value = myDE.Value
                        };
                        parList.Add(par);
                    }
                    command = BuildQueryCommand(conn, strCmdTxt, parList.ToArray());
        ...
        /// <returns>SqlDataReader</returns>
        public SqlDataReader RunProcedureReader(string storedProcName, IDataParameter[] parameters)
        {
            SqlConnection connection = new SqlConnection(ConnectionString);
        ...
        public string ProcedureGetOutPut(string storedProcName, IDataParameter[] parameters, string outputName)
        {
            SqlConnection connection = new SqlConnection(ConnectionString);
            connection.Open();
            SqlCommand command = BuildQueryCommand(connection, storedProcName, parameters);
            command.CommandType = CommandType.StoredProcedure;
            command.ExecuteNonQuery();
            return command.Parameters[outputName].Value.ToString();
        ...
        public DataTable RunProcedureTable(string storedProcName, IDataParameter[] parameters)
        {
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            {
                DataSet dataSet = new DataSet();
